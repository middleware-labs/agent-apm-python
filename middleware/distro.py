    ...
    if isTracker:
        ...
        for entry_point in pkg_resources.iter_entry_points("opentelemetry_instrumentor"):
            try:
                ...
                print(f"Entrypoint instrumentor :", instrumentor_class)
                instrumentor = instrumentor_class()
                # Check if already instrumented (safe access)
                print(f"Is {entry_point.name} instrumented ? {instrumentor.is_instrumented_by_opentelemetry()}")
                if instrumentor.is_instrumented_by_opentelemetry():
                    print(f"Skipping {entry_point.name}: already instrumented")
                    continue
                instrumentor.instrument()
            except Exception as e:
                pass
                # print(f"Failed to instrument {entry_point.name}: {e}")
