...
def create_resource(options: MWOptions):
    ...
    if options.custom_resource_attributes is not None:    
        try:
            extra = {k.strip(): v.strip() for k, v in (item.split("=") for item in options.custom_resource_attributes.split(","))}
            attributes.update(extra)
        except Exception as exc:
            _logger.warning("Skipped Custom attributes: parsing error expected format `abcd=1234,wxyz=5678`")
